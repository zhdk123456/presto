        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Expression>
    {
        ...
        {
            Set<Symbol> deterministicSymbols = node.getAssignmentsMap().entrySet().stream()
            ...
            PlanNode rewrittenNode = context.defaultRewrite(node,
                    ExpressionTreeRewriter.rewriteWith(new ExpressionSymbolInliner(node.getAssignmentsMap()), combineConjuncts(conjuncts.get(true))));
        ...
        public PlanNode visitJoin(JoinNode node, RewriteContext<Expression> context)
        {
            ...
            if (leftSource != node.getLeft() ||
                ...
                output = new JoinNode(node.getId(), node.getType(), leftSource, rightSource, joinConditionBuilder.build(), outputSymbols, newJoinFilter, node.getLeftHashSymbol(), node.getRightHashSymbol(), node.getDistributionType());
            }
