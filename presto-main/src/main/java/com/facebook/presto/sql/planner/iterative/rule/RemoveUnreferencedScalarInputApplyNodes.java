        ...
        implements Rule
{
    ...
    {
        return cast(planNode, ApplyNode.class)
                .filter(applyNode -> canBeReplacedByItsSubquery(applyNode, lookup))
                .map(applyNode -> lookup.resolve(applyNode.getSubquery()));
    }

    public boolean canBeReplacedByItsSubquery(ApplyNode applyNode, Lookup lookup)
    {
        PlanNode input = lookup.resolve(applyNode.getInput());
        PlanNode subquery = lookup.resolve(applyNode.getSubquery());
        return input.getOutputSymbols().isEmpty() && isScalar(input) && isResolvedScalarSubqueryOf(applyNode, subquery);
    }
}
