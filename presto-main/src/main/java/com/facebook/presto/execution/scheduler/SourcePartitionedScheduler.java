        ...
        implements StageScheduler
{
    ...
    public synchronized ScheduleResult schedule()
    {
        ...
        if (!pendingSplits.isEmpty() && state == State.STARTED) {
        ...
        switch (state) {
            ...
            default:
                throw new IllegalStateException(
                        format("SourcePartitionedScheduler expeted to be in STARTED or SCHEDULED state" +
                                " but is in [%s]", state));
        }
    }
    ...
    private ScheduleResult scheduleEmptySplit()
    {
        ...
        Node randomNode = nodes.get(ThreadLocalRandom.current().nextInt(nodes.size()));

        Set<RemoteTask> emptyTask = assignSplits(ImmutableMultimap.of(randomNode, splitSource.getEmptySplit()));
