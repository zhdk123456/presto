        ...
        implements DataDefinitionTask<SetRole>
{
    ...
    @Override
    public CompletableFuture<?> execute(SetRole statement, TransactionManager transactionManager, Metadata metadata, AccessControl accessControl, QueryStateMachine stateMachine, List<Expression> parameters)
    {
        Session session = stateMachine.getSession();
        String catalog = createCatalogName(session, statement, statement.getCatalog());
        if (statement.getType() == SetRole.Type.ROLE) {
            accessControl.checkCanSetRole(session.getRequiredTransactionId(), session.getIdentity(), statement.getRole().get(), catalog);
        }
        ...
        stateMachine.addSetRole(catalog, new SelectedRole(type, statement.getRole()));
