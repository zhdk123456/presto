            ...
            try {
                ...
                if (groupDistinguishedName.isPresent()) {
                    if (!ldapBinder.checkForGroupMembership(user, groupDistinguishedName.get(), context)) {
                        String errorMessage = format("User %s not a member of the group %s.", user, groupDistinguishedName.get());
                        LOG.debug("Authorization failed for user. " + errorMessage);
                        throw new RuntimeException("Unauthorized User: " + errorMessage);
                    }
                    LOG.debug("Authorization succeeded for user %s in group %s.", user, groupDistinguishedName.get());
                }
