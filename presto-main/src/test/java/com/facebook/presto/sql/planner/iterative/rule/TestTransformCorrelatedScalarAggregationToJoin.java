...
public class TestTransformCorrelatedScalarAggregationToJoin
{
    ...
    private AggregationNode createSumAggregation(PlanBuilder p, Symbol symbol, List<List<Symbol>> groupingSets, PlanNode source)
    {
        ...
        AggregationNode.Aggregation aggregation = new AggregationNode.Aggregation(functionCall, signature);

        return p.aggregate(ImmutableMap.of(p.symbol("sum", BIGINT), aggregation), groupingSets, source);
    }
}
