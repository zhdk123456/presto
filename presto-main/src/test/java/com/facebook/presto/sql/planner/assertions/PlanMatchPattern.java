...
public final class PlanMatchPattern
{
    ...
    {
        return node(JoinNode.class, left, right).with(new JoinMatcher(
                joinType,
                expectedEquiCriteria,
                expectedFilter.map(predicate -> new SqlParser().createExpression(predicate)),
    ...
    public static FunctionCall functionCall(String name, SymbolReference... args)
    {
        return new FunctionCall(QualifiedName.of(name), Optional.empty(), false, Arrays.asList(args));
    }
