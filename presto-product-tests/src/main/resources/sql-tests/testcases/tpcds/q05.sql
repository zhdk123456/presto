  ...
  (SELECT s_store_id,
   ...
   FROM
     (SELECT ss_store_sk AS store_sk,
      ...
      FROM "tpcds"."sf1".store_returns) salesreturns,
        "tpcds"."sf1".date_dim,
        "tpcds"."sf1".store
   WHERE date_sk = d_date_sk
     AND cast(d_date as date) BETWEEN cast('2000-08-23' AS date) AND (cast('2000-08-23' AS date) + INTERVAL '14' DAY)
  ...
  (SELECT cp_catalog_page_id,
   ...
   FROM
     (SELECT cs_catalog_page_sk AS page_sk,
      ...
      FROM "tpcds"."sf1".catalog_returns) salesreturns,
        "tpcds"."sf1".date_dim,
  ...
  (SELECT web_site_id,
   ...
   FROM
     (SELECT ws_web_site_sk AS wsr_web_site_sk,
      ...
      LEFT OUTER JOIN "tpcds"."sf1".web_sales ON (wr_item_sk = ws_item_sk
