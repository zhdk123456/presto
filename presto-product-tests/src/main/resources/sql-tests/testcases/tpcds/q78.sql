...
WITH ws AS
  (SELECT d_year AS ws_sold_year,
   ...
   LEFT JOIN "tpcds"."sf1".web_returns ON wr_order_number=ws_order_number
   AND ws_item_sk=wr_item_sk
   JOIN "tpcds"."sf1".date_dim ON ws_sold_date_sk = d_date_sk
...
cs AS
  (SELECT d_year AS cs_sold_year,
   ...
   LEFT JOIN "tpcds"."sf1".catalog_returns ON cr_order_number=cs_order_number
   AND cs_item_sk=cr_item_sk
   JOIN "tpcds"."sf1".date_dim ON cs_sold_date_sk = d_date_sk
...
ss AS
  (SELECT d_year AS ss_sold_year,
   ...
   LEFT JOIN "tpcds"."sf1".store_returns ON sr_ticket_number=ss_ticket_number
   AND ss_item_sk=sr_item_sk
   JOIN "tpcds"."sf1".date_dim ON ss_sold_date_sk = d_date_sk
